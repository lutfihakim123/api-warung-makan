...
const (
	...
	SelectAllNota    = `SELECT nota_penjualan.Id as id, mst_pelanggan.id as pelanggan_id, mst_karyawan.id as karyawan_id, mst_meja.id as meja_id,  mst_meja.no_meja as no_meja, rincian.menu_id as menu_id, mst_pelanggan.nama as nama_pelanggan, mst_karyawan.nama as nama_karyawan, mst_menu.nama as nama_menu, mst_menu.harga as harga_menu, rincian.kuantitas as kuantitas, nota_penjualan.waktu_pesan as waktu_pesan, rincian.total as total, mst_menu.img as image_menu, rincian.created_at as created_at, rincian.updated_at from mst_menu left join rincian on mst_menu.id=rincian.menu_id right join nota_penjualan ON rincian.nota_id=nota_penjualan.id  inner join mst_pelanggan ON mst_pelanggan.id = nota_penjualan.pelanggan_id inner join mst_karyawan ON mst_karyawan.id = nota_penjualan.karyawan_id inner join mst_meja on mst_meja.id=nota_penjualan.meja_id limit $1 offset $2`
